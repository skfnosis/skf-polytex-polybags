-- ============================================================================
-- Manual "Color" field on fabric line items (Purchase/Sale Order, Bill,
-- Return). Free text, set per line, fabric category only in the UI — but
-- stored generically on invoice_items so nothing stops it being used
-- elsewhere later.
-- ============================================================================

alter table public.invoice_items add column color text;
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.invoice_items (
      ...
      reserved_for_party_id, fulfilled_from_item_id, color
    )
    values (v_invoice_id,
