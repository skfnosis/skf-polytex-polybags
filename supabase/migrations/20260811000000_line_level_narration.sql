-- ============================================================================
-- LINE-LEVEL NARRATION — narration now belongs to each purchase/sale line,
-- not just the bill as a whole (different lines can represent different
-- orders/purposes). invoices.narration is left in place, untouched, for
-- backward compatibility with existing rows; the UI simply stops writing to
-- it and writes per-line narration instead. Same create_invoice signature —
-- narration travels as an extra key on each element of the existing
-- p_items jsonb array, so old callers that omit it just get a null line
-- narration, nothing breaks.
-- ============================================================================

alter table public.invoice_items add column narration text;
