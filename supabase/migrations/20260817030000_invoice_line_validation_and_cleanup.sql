-- ============================================================================
-- SECURITY / DATA-INTEGRITY FIX: create_invoice / update_invoice never
-- validated that line quantity/rate were sane numbers.
-- ============================================================================
-- The app's own entry forms filter to `Number(l.quantity) > 0 && Number(l.rate)
-- >= 0` before submitting, but that is a client-side convenience, not a
-- guarantee -- create_invoice/update_invoice are plain RPCs any authenticated
-- user with entry_sale/entry_purchase 'create'/'approve' can call directly
-- with any payload. A negative quantity or rate would silently flip the sign
-- of the posted ledger entries and stock movements (e.g. a "purchase" that
-- actually reduces recorded stock and reverses which side of the ledger gets
-- debited), letting a bad line quietly corrupt the books instead of failing
-- loudly. create_expense/record_payment already guard their amount the same
-- way ("Amount must be positive") -- invoices never got the equivalent line
-- check. Fixed by validating every line before any insert happens.
--
-- Also drops the 3 older create_invoice() overloads left over from earlier
-- migrations that only added optional trailing params via CREATE OR REPLACE
-- with a different signature (so Postgres kept the old ones as separate
-- overloads instead of replacing them -- the same pattern that caused the
-- unprotected create_party() overload fixed in the previous migration). All
-- 3 already have their own has_permission() checks, so this isn't a
-- privilege-escalation gap like create_party was, but leaving them around
-- means any future validation added only to the newest signature (like this
-- one) can be sidestepped by calling an older signature directly. The app's
-- JS wrapper always sends all 14 params, so only the 14-arg version is ever
-- actually used -- the other 3 are dead code kept alive only as attack
-- surface.
-- ============================================================================

drop function if exists public.create_invoice(text, text, text, uuid, date, jsonb);
drop function if exists public.create_invoice(text, text, text, uuid, date, jsonb, text, uuid, numeric, numeric, numeric, numeric);
drop function if exists public.create_invoice(text, text, text, uuid, date, jsonb, text, uuid, numeric, numeric, numeric, numeric, text);
